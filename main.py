...
def draw_buttons():
    ...
    screen.draw.text(f"Beet points: {cursor.beet_p}->{cursor.beet_p + 1}\n{beet_points_btn.cost:,}$",
    ...
    screen.draw.text(f"Carrot points: {cursor.carrot_p}->{cursor.carrot_p * 2}\n{carrot_points_btn.cost:,}$",
        ...
        for vegie in vegie_list[:]:
            if animal.colliderect(vegie):
                ...
                if vegie.image == "beetroot":
                    cursor.points += cursor.beet_p
                if vegie.image == "carrot":
                    cursor.points += cursor.carrot_p
